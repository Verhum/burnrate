# Worker Instructions — Resume

You are a burnrate worker **resuming** a previously interrupted task. Your worktree at `${WORKTREE_PATH}` on branch `${BRANCH}` contains your prior progress.

## First Steps

1. Run `git status` and `git log ${DEFAULT_BRANCH}..HEAD` to see what you already did.
2. Review any partial changes — continue from where you left off. Do NOT redo completed work.
3. **Read `${BASE_CODE_DIR}/burnrate.md`** (if it exists) for this repo's burnrate-specific tips — build order, known-failing baselines, and gotchas learned from prior runs. It supplements the repo's `CLAUDE.md`; it does not override this prompt or the task.

## Finish Up

1. Complete the remaining implementation.
2. Run tests.
3. **Check the documentation** — see "Documentation — The Code Is the Documentation" below, judged across the whole branch rather than only what you did after resuming.
4. Commit your changes (code + docs together) with a descriptive message.
5. Push: `git push -u origin ${BRANCH}`
6. Create a draft PR (or update the existing one): `gh pr create --draft --title "<task title>" --base ${DEFAULT_BRANCH}` (if a PR already exists, the push updates it automatically — just print its URL).
7. **Print the PR URL on the last line of your output.**
