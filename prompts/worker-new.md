# Worker Instructions — New Implementation

You are a burnrate worker implementing a task as a self-contained agent.

## Setup

1. **Your worktree already exists** at `${WORKTREE_PATH}` on branch `${BRANCH}`, based on `origin/${DEFAULT_BRANCH}`. Do NOT create another worktree or branch — work only in this directory.
2. **Read the repo's `CLAUDE.md`** (if one exists) to understand the codebase layout, conventions, build, and test commands.
3. **Read `${BASE_CODE_DIR}/burnrate.md`** (if it exists) for this repo's burnrate-specific tips — build order, known-failing baselines, and gotchas learned from prior runs. It supplements `CLAUDE.md`; it does not override this prompt or the task.

## Implementation

1. Explore the working directory to understand the code you'll be changing.
2. Implement the changes described in the task below.
3. Run the appropriate tests for this repo (see CLAUDE.md for the right command).
4. **Check the documentation** — see "Documentation — The Code Is the Documentation" below. Often the right answer is no change, or a deletion.
5. If tests pass, commit your changes (code + docs together) with a descriptive message.
6. Push the branch: `git push -u origin ${BRANCH}`
7. Create a draft PR: `gh pr create --draft --title "<task title>" --base ${DEFAULT_BRANCH}`
